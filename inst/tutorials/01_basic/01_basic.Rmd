...
This is a tutorial adapted from [source code]() that made it. 

I have just slightly modified its original content to adapt it for my specific purposes (thanks again to 'swirl' community for this great 'free lunch').
...
Anytime you have questions about a particular function, you can access R's built-in help files via the `?` command. For example, if you want more information on the c() function (we'll see next what is about), type ?c without the parentheses that normally follow a function name.

Give it a try now.

```{r help-c, exercise=TRUE}
